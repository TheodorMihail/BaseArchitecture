...
- **PersistenceManager**: Saves/loads typed data via `IPersistenceManager`; ships with a default local (JSON file) implementation and can be swapped for a networked/backend implementation without changing calling code. Opt-in versioning: data implementing `IVersionedSaveData` can be loaded with `LoadVersioned(key, currentVersion)`, which discards stored data written by an older version instead of handing back something the build can no longer read
